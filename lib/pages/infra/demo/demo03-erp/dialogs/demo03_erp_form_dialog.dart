...
/// 学生表单对话框 - ERP模式（不包含子表）
class Demo03ErpFormDialog extends StatefulWidget {
  final Demo03Student? student;
  final WidgetRef ref;
  final VoidCallback onSuccess;

  const Demo03ErpFormDialog({
    super.key,
    this.student,
    required this.ref,
  ...
  @override
  State<Demo03ErpFormDialog> createState() => _Demo03ErpFormDialogState();
}

class _Demo03ErpFormDialogState extends State<Demo03ErpFormDialog> {
  ...
  Future<void> _loadStudentDetail() async {
    ...
    try {
      final studentApi = widget.ref.read(demo03StudentErpApiProvider);
...
/// 显示学生表单对话框的便捷方法 - ERP模式
void showDemo03ErpFormDialog(
  ...
  showDialog(
    context: context,
    builder: (context) => Demo03ErpFormDialog(
      student: student,
      ref: ref,
      onSuccess: onSuccess,
    ),
